...
// These are the names of the folders inside of the exercises folder inside
// of the exercises.json repository on GitHub:
...
/**
 * A function that looks up the first exercise with the search term and returns
 * the index of that exercise in the exercises array.
 * 
 * @param {string} search : a search term for an exercise (e.g. "hamstring")
 */
function lookupExercise(search) {
    return exercises.findIndex(
        exerciseName => exerciseName.toLowerCase().includes(search.toLowerCase())
    );
}

/**
 * A function that gives back the raw URL of the exercise's JSON file so it can
 * be fetched by the Fetch API in the browser.
 * 
 * @param {number} index : a number given back by the lookupExercise function
 */
function getExerciseJsonUrl(index) {
    return `https://raw.githubusercontent.com/wrkout/exercises.json/master\
/exercises/${exercises[index]}/exercise.json`;
}

// debugger;
// // this will give me the raw JSON URL for the hamstring exercis
// getExerciseJsonUrl(
//     lookupExercise("bicep")
// );